...
# ---------------------------
# Prediction Function Using Template - FIXED VERSION
# ---------------------------
def predict_using_template(row_data, model, sample_data, features, X_dtypes):
    """Use the sample data from Colab as a template for prediction"""
    try:
        # Create a new DataFrame based on the sample template
        prediction_row = sample_data.iloc[0:1].copy()
        
        # Set all feature values to 0 to start fresh
        for col in prediction_row.columns:
            prediction_row[col] = 0
        
        # CRITICAL FIX: Set the style number column to 1
        if 'Style Number' in row_data:
            ...
            if style_col in prediction_row.columns:
                ...
                if style_cols:
                    ...
                    # In a real application, you might want better matching logic
                    prediction_row[style_cols[0]] = 1
                    if st.session_state.debug_mode:
                        st.write(f"Using fallback style column: {style_cols[0]}")
        
        # Fill in other categorical columns using similar logic
        # For example: Color Combo, Category, etc.
        for cat_feature in ['Color Combo', 'Category', 'Sub Category', 'Size']:
            if cat_feature in row_data:
                col_name = f"{cat_feature}_{row_data[cat_feature]}"
                if col_name in prediction_row.columns:
                    prediction_row[col_name] = 1
        
        # Fill in the lag features and other numeric values
        ...
        prediction_row = prediction_row.astype(X_dtypes)
        
        # Make prediction with the template
        pred = model.predict(prediction_row)[0]
    ...
    with st.spinner("Generating predictions - this may take a moment..."):
        ...
        for idx, row in last_obs.iterrows():
            ...
            for i in range(3):  # 3 months
                try:
                    # Use template-based prediction
                    pred = predict_using_template(current_state, model, sample_data, features, X_dtypes)
